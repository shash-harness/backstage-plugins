...
const useSLOList = ({
  ...
  useAsyncRetry(async () => {
    ...
    const response = await fetch(
      `${await backendBaseUrl}/harness/${env}/gateway/cv/api/slo-dashboard/widgets/list?routingId=${accountId}&pageNumber=0&pageSize=40&accountId=${accountId}&orgIdentifier=${orgId}&projectIdentifier=${projectId}&monitoredServiceIdentifier=${monitoredServiceId}`,
      {
        method: 'GET',
        headers,
      },
